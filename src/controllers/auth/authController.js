...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (existedUser) throw new ApiError(409, "User with same email or phone number already exists");
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!(email || phoneNo)) throw new ApiError(400, "Email or phone number is required");

  const user = await User.findOne({ $or: [{ email }, { phoneNo }] });
  if (!user) throw new ApiError(401, "Invalid email or phone number");
...
const UpdateAccountDetail = asyncHandler(async (req, res) => {
  ...
  io.emit("user", { action: "update", user, message: `User ${user._id} updated successfully` });
...
const getCurrentUser = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id).select("-password").populate("tradeHistory");
